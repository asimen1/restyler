...
function onInspectClick(e) {
    ...
    let getInspectValue = function($inspectedElement) {
        let retVal = '';

        retVal += $inspectedElement.prop('tagName');

        let id = $inspectedElement.attr('id');
        if (id) {
            retVal += '#' + id;
        }

        let classesStr = $inspectedElement[0].className;
        if (classesStr && typeof classesStr === 'string') {
            let classesArr = classesStr.split(/\s+/);
